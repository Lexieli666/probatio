...
class ValidationRecord(BaseModel):
    ...
    Attributes:
        ...
        rows_reasked: How many rows had to be asked again because the judge's first reply was not a
            verdict at all (DECISIONS 92). ``0`` in ``columns`` mode and in any run where every
            first reply parsed. A row is asked at most
            :data:`~probatio.cli.JUDGE_ATTEMPTS` times before the run stops, so this can never
            exceed ``n``. A record with a high count measured a judge that is hard to get an
            answer out of, which is a fact about the judge and belongs beside its kappa.
